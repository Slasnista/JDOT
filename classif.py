...
def loss_hinge(Y, F):
    ...
    for i in range(Y.shape[1]):
        res += np.maximum(0, 1 - Y[:, i].reshape((Y.shape[0], 1)) *
                          F[:, i].reshape((1, F.shape[0])))**2
    return res
...
class Classifier:
    ...
    def crossval(self, X, Y, kerneltype='linear', nbsplits=5,
                 g_range=np.logspace(-3, 3, 7), l_range=np.logspace(-3, 0, 4)):
...
def hinge_squared_reg(w, X, Y, lambd):
    """
    compute loss dans gradient for squared hing loss
    ...
    for i in range(nbclass):
        loss += Y[:, i].T.dot(err_alpha[:, i] ** 2) + \
            (1 - Y[:, i]).T.dot(err_alpha1[:, i]**2)

        grad[:, i] += 2 * X.T.dot(-Y[:, i] * err_alpha[:, i] +
                                  (1 - Y[:, i]) * err_alpha1[:, i])
...
def hinge_squared_reg_bias(w, X, Y, lambd):
    """
    compute loss dans gradient for squared hing loss with
    ...
    grad = np.zeros_like(w)
    for i in range(nbclass):
        loss += Y[:, i].T.dot(err_alpha[:, i] ** 2) + (1 - Y[:, i]).T.dot(
            err_alpha1[:, i]**2)
        grad[:, i] += 2 * X.T.dot(-Y[:, i] * err_alpha[:, i] +
                                  (1 - Y[:, i]) * err_alpha1[:, i])
...
class SVMClassifier(Classifier):
    ...
    def fit(self, K, y):
        # beware Y is a binary matrix to allow for more
        # general solvers (see JDOT)

        if self.bias:
            K1 = np.hstack((K, np.ones((K.shape[0], 1))))
            self.w = np.zeros((K1.shape[1], y.shape[1]))
            self.w, self.f, self.log = spo.fmin_l_bfgs_b(
                lambda w: hinge_squared_reg_bias(
                    w, X=K1, Y=y, lambd=self.lambd),
            ...
            self.w, self.f, self.log = spo.fmin_l_bfgs_b(
                lambda w: hinge_squared_reg(
                    w, X=K, Y=y, lambd=self.lambd),
                self.w, maxiter=1000, maxfun=1000)
...
class KRRClassifier(Classifier):
    ...
    def fit(self, K, y, sw=False):
        ...
        if sw:
            K = K * sw
        K0 = np.vstack(
            (np.hstack((np.eye(ns), np.zeros((ns, 1)))),
             np.zeros((1, ns + 1))))
