...
# y: source labeks
# Xtest: target domain
# ytest is optionnal, just to measure
#     performances of the method along iterations
...
def jdot_krr(X, y, Xtest, gamma_g=1, max_iter_bcd=10, alpha=1, lambd=1e1,
             method='emd', reg=1, ktype='linear'):
    """jdot_krr

    Parameters:
    -----------
    X : array-like, shape (n_source_samples, n_features)
        The array of source samples
    y : array-like, shape (n_source_samples,)
        The array of source labels
    Xtest : array-like, shape (n_target_samples, n_features)
        The array of target samples
    gamma_g : float, optional (default=1.)
        The RBF rernel param
    max_iter_bcd : int, optional (default=10)
        The number of iterations for block coordinates descent
    alpha : float, optional (default=1.)
        The weighting parameter between the OT cost function and
        the classification cost function
    method : string, optional (default="emd")
        The algorithm to use for OT minimization step
    ktype : string, optional (default="linear")
        The kernel type

    Returns:
    --------
    g : sklearn classifier instance
    ...
    # compute kernels
    if ktype == 'rbf':
        Kt = sklearn.metrics.pairwise.rbf_kernel(
            Xtest, Xtest, gamma=gamma_g)
    else:
        Kt = sklearn.metrics.pairwise.linear_kernel(
            Xtest, Xtest)

    C = alpha * C0  # + cdist(y,ypred,metric='sqeuclidean')
    k = 0
    while (k < max_iter_bcd):  # and not changeLabels:
        k = k + 1
        if method == 'sinkhorn':
...
def jdot_svm(X, y, Xtest,
             ytest=[], gamma_g=1, max_iter_bcd=10, alpha=1,
    ...
    Xtest : array-like, shape (n_target_samples, n_features)
        The array of target samples
    ytest : array-like, shape (n_target_samples)
    ...
    alpha : float, optional (default=1.)
        The weighting parameter between the OT cost function and
        the classification cost function
    lambd : float, optional (default=1e1)
        The regularization parameter for SVM
    method : string, optional (default="emd")
        The algorithm to use for OT minimization step
    reg_sink : float, optional (default=1.)
        The regularization parameter to use for Sinkhorn algorithm
    ktype : string, optional (default="linear")
        The kernel type
    ...
    g : sklearn classifier instance
        The trained classifier
    results : dict
        The dictionary of training history
...
def jdot_nn_l2(get_model, X, Y, Xtest, ytest=[], fit_params={},
               reset_model=True, numIterBCD=10, alpha=1, method='emd',
               reg_sink=1, nb_epoch=100, batch_size=10):
    """jdot_nn_l2

    Parameters:
    -----------
    get_mode : sklearn classifier instance
        A given classifier (nn) compited with l2 loss
    X : array-like, shape (n_source_samples, n_features)
        The array of source samples
    y : array-like, shape (n_source_samples,)
        The array of source labels
    ...
    fit_params : dict, optional (default={})
        The dictionary of classifier parameters
    reset_model : boolean, optional (default=True)
        Whether the function create a new model
    gamma_g : float, optional (default=1.)
        The RBF rernel param
    max_iter_bcd : int, optional (default=10)
        The number of iterations for block coordinates descent
    ...
    reg_sink : float, optional (default=1.)
        The regularization parameter to use for Sinkhorn algorithm
    nb_epoch : int, optional (default=100)
