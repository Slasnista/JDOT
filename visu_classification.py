...
def plot_data_classif(X, y, Z=None):
    ...
    if Z is not None:
        pl.pcolormesh(
            xx, yy, np.argmax(Z, 2),
            edgecolors='face', alpha=.1, vmin=0, vmax=2)
    pl.scatter(
        X[:, i1], X[:, i2], c=y, edgecolors='black')
...
def predict_test_grid(clf, gamma, Xapp):
    return predict_test(clf, gamma, Xapp, xfin).reshape(
        (xx.shape[0], xx.shape[1], 3))
...
clf_jdot, dic = jdot.jdot_svm(
    X, Y, Xtest, ytest, gamma_g=gamma, max_iter_bcd=itermax,
    alpha=.1, lambd=reg, ktype='rbf')
...
for i in range(10):
    clf_jdot, res = jdot.jdot_svm(
        X, Y, Xtest, ytest,
        gamma_g=gamma, max_iter_bcd=i + 1, alpha=alpha,
        lambd=reg, ktype='rbf')
